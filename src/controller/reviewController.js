...
const createReview = async function (req, res) {
   try {
        ...
        if (!isValid(reviews)) { return res.status(400).send({ status: false, message: "enter review" }) }

        let bookreviews = /\w*\s*|\w|\D/.test(reviews.trim())
        if (!bookreviews) return res.status(400).send({ status: false, message: "enter valid review" })
        }
...
const deleteReview = async function (req, res) {
    try {
        ...
        if (book === null) {
            return res.status(404).send({ status: false, message: 'No such book found with the given bookId' });
        }

        if (!reviewId) return res.status(400).send({ status: false, message: "reviewId is required" })

        var isValidId = mongoose.Types.ObjectId.isValid(reviewId)
        if (!isValidId) return res.status(400).send({ status: false, message: "Enter valid review id" })

        let review = await reviewModel.findOne({ _id: reviewId, bookId: bookId, isDeleted: false });
        if (review === null) {
            return res.status(404).send({ status: false, message: 'No such review found  with the given input data' });
        }
